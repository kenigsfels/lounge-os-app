...
function renderCategories(items) {
  return WAREHOUSE_CATEGORIES.map((category) => {
    ...
    return `<section class="stock-category"><header><div><small>Категория</small><h2>${category.label}</h2></div><span>${categoryItems.length}</span></header><div>${categoryItems.length ? categoryItems.map(renderItem).join('') : '<p class="stock-category__empty">Пока нет позиций</p>'}</div></section>`;
  }).join('');
}
...
export function initWarehouseScreen(root,{showToast=()=>{}}={}){
  ...
  screen.addEventListener('click',onClick);drawer.addEventListener('submit',onSubmit);return()=>{if(drawer.open)drawer.close();screen.removeEventListener('click',onClick);drawer.removeEventListener('submit',onSubmit)};
}
